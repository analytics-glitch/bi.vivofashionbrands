...
- **Flat table** (`SortableTable`): new "Cover (wks)" column with row-level pill (color: rose<4w · emerald 4–17w · amber>17w · italic "Idle" when no in-window sales) and a hover tooltip showing the formula `Weeks of Cover = Stock Units ÷ (Units Sold ÷ N weeks)` and the substituted numbers. New "SOR %" column with formula tooltip `Units Sold ÷ (Units Sold + Stock) × 100`. Total footer row uses `SortableTable.footerRow` prop with 10 cells (Total label + Units Sold + Inventory Units + % Units Sales + % Units Inventory + Cover + SOR + Variance pts + Risk).
- **Grouped accordion** (`CategoryAccordionTable`): grid widened 8 → 10 cols. Top header strip + per-category row + inner sub-row + accordion-total footer all in lock-step. Cover and SOR pills on category rolls AND each sub-row. CSV export columns expanded accordingly. Header label updates: "Inventory Units", "% Units Sales", "% Units Inventory". New `accordion-total-row` testid for QA.
- **SortableTable**: extended column schema to support `headerTitle` → `<th title>` so callers can attach formula tooltips to column headers.
- **Verified live (preview)**: by-Category table gone; by-Subcategory shows Window + Custom + Flat/Grouped toggle. Live totals at default 30d window: Units Sold 23,655 · Inventory Units 73,730 · % Sales 99.99% · % Inv 98.61% · Cover **13.4w** · SOR **24.3%** · Variance +1.38 pts.
